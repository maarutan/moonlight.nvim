...
if os.linux then
	...
	function Trash_cli:last_restorable_name()
		if not which:is_exists("trash-restore") then return nil end
...
vim.api.nvim_create_autocmd("User", {
	pattern = { "TrashPut", "TrashRestore" },
	callback = function()
		local ok, manager = which:is_module_has("neo-tree.sources.manager")
		if ok then manager.refresh() end
	end,
})
