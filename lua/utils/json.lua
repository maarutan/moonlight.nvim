...
local function ensure_dir_exists(path)
	...
	if dir then
		if vim and vim.fn and vim.fn.isdirectory(dir) == 0 then
			vim.fn.mkdir(dir, "p")
		else
...
---@return string
function json.encode_pretty(val, indent, level)
	local indent_size = indent or 2
	local lvl = level or 0
	local pad = string.rep(" ", indent_size * lvl)
	local pad_inner = string.rep(" ", indent_size * (lvl + 1))
	...
	if t == "table" then
		...
		if is_array then
			for _, v in ipairs(val) do
				table.insert(parts, pad_inner .. json.encode_pretty(v, indent_size, lvl + 1))
			end
			return "[\n" .. table.concat(parts, ",\n") .. "\n" .. pad .. "]"
		else
			for k, v in pairs(val) do
				table.insert(parts, pad_inner .. '"' .. tostring(k) .. '": ' .. json.encode_pretty(v, indent_size, lvl + 1))
			end
			return "{\n" .. table.concat(parts, ",\n") .. "\n" .. pad .. "}"
		end
	elseif t == "string" then
		return '"' .. val:gsub('[%z\1-\31\\"]', function(c) return escape_char_map[c] or string.format("\\u%04x", c:byte()) end) .. '"'
...
function json.save(path, tbl, indent)
	ensure_dir_exists(path)
	local data = indent and json.encode_pretty(tbl, indent) or json.encode(tbl)
	local f, err = io.open(path, "w")
	if not f then error("Failed to open file: " .. tostring(err)) end
	f:write(data)
	f:close()
end
...
--- Merge new_data into existing JSON file
function json.update(path, new_data, indent)
	local data = json.load(path)
	for k, v in pairs(new_data) do
		data[k] = v
	end
	json.save(path, data, indent)
end
