...
function M.toggle_word()
	...
	if not word or word == "" then
		vim.notify(const.icons.diagnostic.warn .. " No word under cursor", vim.log.levels.WARN)
		return
	end

	local base_new = find_toggle_pair(word)
	if not base_new then
		vim.notify(const.icons.diagnostic.warn .. " Can't toggle: " .. word, vim.log.levels.WARN)
		return
	end
