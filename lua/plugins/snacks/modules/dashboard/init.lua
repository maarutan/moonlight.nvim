	...
	preset = {
		...
		keys = {
			...
			},
			{
				...
				action = function() Snacks.picker.recent({ filter = { cwd = true }, confirm = { "edit", "tcd" } }) end,
			},
			{
				icon = icons.word,
				key = "w",
			...
			},
			{
				...
				key = "c",
				desc = " _  󱦰 Config",
				action = function() Snacks.picker.files({ cwd = vim.fn.stdpath("config"), confirm = { "edit", "tcd" } }) end,
			},
			{
				icon = icons.exit,
				key = "q",
				...
				action = function()
					vim.ui.input({ prompt = "Do you want to quit? [y/N] " }, function(input)
						...
						input = input:lower()
						if input == "y" or input == "yes" then vim.cmd("qall!") end
					end)
				end,
			},
		},
