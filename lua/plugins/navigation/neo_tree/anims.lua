...
local function find_neotree_win()
	for _, win in ipairs(vim.api.nvim_list_wins()) do
		...
		if name:match("neo%-tree") then return win, buf end
	end
end
...
function M.start()
	...
	if mode == "float" then anim_width = math.floor(nt_width * float_width_scale) end
	...
	if mode == "float" then
		...
		local row_offset = math.max((nt_height - anim_height - 1) + position_offset, 0)
	...
	timer:start(0, delay, function()
		vim.schedule(function()
			...
			if anim_win and vim.api.nvim_win_is_valid(anim_win) then
				local new_w = (mode == "float") and math.floor(vim.api.nvim_win_get_width(nwin2) * float_width_scale) or vim.api.nvim_win_get_width(nwin2)
...
function M.stop()
	...
	if anim_win and vim.api.nvim_win_is_valid(anim_win) then vim.api.nvim_win_close(anim_win, true) end
